...
fn convert_to_oklab(egui: &Color) -> Oklab {
    let rgba = egui::Rgba::from(egui.clone());
...
fn interpolate(start: &Oklab, end: &Oklab, counts_minus_one: f32, i: f32) -> Color {
    ...
    let interpolated = convert_from_oklab(oklab);
    interpolated
}
    ...
    pub fn with_options(options: &ColorGradientOptions) -> Self {
        Self(match options {
            ...
            ColorGradientOptions::StartCenterEnd {
                ...
                0 => vec![],
                1 => vec![center.clone()],
                2 => vec![start.clone(), end.clone()],
                3 => vec![start.clone(), center.clone(), end.clone()],
                n if n % 2 == 0 => {
                    let steps = n;
