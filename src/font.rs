        ...
        match &font {
            Font::EguiMonospace => {
                let font = fonts
                    ...
                    .flatten()
                    .map(|font| rusttype::Font::try_from_bytes(&font.font))
                    .flatten()
            ...
            Font::Font8x8 => {
                ...
                for c in text.chars() {
                    ...
                    for column in 0..8 {
                        ...
                        for row in 0..8 {
                            c[row] = match bitmap[row] & 1 << column {
                                0 => false,
                                _ => true,
                            }
                        }
                    ...
                    while columns.last().map(|x| x.iter().all(|x| *x == false)) == Some(true) {
                        columns.pop();
                    }
                    while columns.first().map(|x| x.iter().all(|x| *x == false)) == Some(true) {
                        columns.remove(0);
                    }
                    if !columns.is_empty() {
                        chars.push(columns);
