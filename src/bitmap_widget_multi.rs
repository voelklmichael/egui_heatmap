...
impl<Key: Clone + PartialEq + Debug> MultiBitmapWidget<Key> {
    ...
    pub fn ui(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        let image = image.context_menu(|ui| {
            ui.vertical(|ui| {
                ...
                if self.showmap.has_hidden() && ui.button("Show all").clicked() {
    ...
    fn copy_to_clipboard(&self, size: [f32; 2]) {
        let width = size[0] as usize;
        let height = size[1] as usize;
        match self.showmap.render(width, height) {
            Ok(data) => {
